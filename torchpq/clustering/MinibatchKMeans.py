...
class MinibatchKMeans(CustomModule):
  ...
  def get_labels(self, data, centroids):
    ...
    if False:
      ...
      if False: # data.device.type == "cuda":
        if self.distance == "cosine":
      ...
      elif True: #data.device.type == "cpu":
  ...
  def fit_minibatch(self, data, centroids=None):
    ...
    assert data.shape[1] >= self.n_clusters, f"number of samples in minibatch ({data.shape[1]}) should be greater than number of clusters ({self.n_clusters})"

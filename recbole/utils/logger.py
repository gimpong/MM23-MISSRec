...
def init_logger(config):
    ...
    ##############
    suffix = '{}_{}'.format(get_local_time(), config['log_suffix']) if 'log_suffix' in config and len(config['log_suffix']) > 0 else get_local_time()
    prefix = '{}_{}'.format(config['model'], config['log_prefix']) if 'log_prefix' in config and len(config['log_prefix']) > 0 else config['model']
    log_dir = os.path.join(LOGROOT, prefix)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logfilename = '{}/{}.log'.format(prefix, suffix)
    ##############
